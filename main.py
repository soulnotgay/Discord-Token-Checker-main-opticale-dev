...
class TokenCheker:
    ...
    async def check_token(self, token: str) -> None:
        """Cette fonction essaiera de se connecter à l'api discord en utilisant un token,

        Cette fonction respectera la limite de taux de discord : `60` par seconde.
        """
    ...
    async def _run(self) -> None:
        ...
        for token in self.load_tokens(q2): 
            ...
            if self._requests_count >= self._rate_limit:
                print(i, "\nLa limite de débit était active en attendant 1 seconde...\n")
                self._requests_count = 0
                await asyncio.sleep(1)

            tasks.append(asyncio.create_task(self.check_token(token)))
            print("{}Total vérifier: {} | Fonctionne: {} | Ne fonctionne pas: {}".format(i, self.counter_working + self.counter_not_working, self.counter_working, self.counter_not_working), end="\r")
        await asyncio.wait(tasks)
        print("{}Total vérifier: {} | Fonctionne: {} | Ne fonctionne pas: {}".format(i, self.counter_working + self.counter_not_working, self.counter_working, self.counter_not_working))

if __name__ == '__main__':
    cls()
    print(f"Discord Token Cheker 1.0\nMade By: {__author__}\n\n")
    q2 = input("{}Quels Tokens voulez-vous vérifier ?[file.txt]: ".format(q))

    if not path.exists(q2):
        while True:
            q2 = input("{}Veuillez mettre un fichier valide!: ".format(x))
            if path.exists(q2):
                break

    q1 = input("{}Vous voulez enregistrer tous les Tokens travaillés dans un fichier txt?[Y/N]: ".format(q)).lower()

    if q1 == "y":
        q3 = input("{}Quel fichier tu veux sauvegarder?[file.txt]: ".format(q))
        if not path.exists(q2):
            while True:
                q3 = input("{}Veuillez mettre un fichier valide!: ".format(x))
                if path.exists(q2):
                    break
